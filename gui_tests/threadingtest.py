...
boardWin: BoardWin | None = None
statsWin: StatsWin | None = None
gameClock: GameClock | None = None
...
def process_new_event(event):
  global  boardWin, statsWin, gameClock, gameFinished
  if event['type'] == 'gameStart':
    gameFinished = False
    # gameClock = GameClock()
...
def configure_gameclock(event):
  ...
  gameClock = GameClock(wtime, winc, btime, binc, side="black") 
...
def process_gameState_event(event):
  ...
  if status == "started":
    ...
    statsWin.set_winner_displayable_stats(event['winner'], win_msg("mate", event['winner']))
    statsWin.redraw()

  elif status == "resign":
    statsWin.set_winner_displayable_stats(event['winner'], win_msg("resign", event['winner']))
    statsWin.redraw()
  elif status == "outoftime":
    statsWin.set_times(wtime=event['wtime'], btime=event['btime'])
    statsWin.set_winner_displayable_stats(event['winner'], win_msg("outoftime", event['winner']))
    statsWin.redraw()
...
def win_msg(type: Literal['resign', 'mate', 'outoftime'], winner: Literal['white', 'black']):
  loser = "white"
  if winner == 'white':
    loser = 'black'
