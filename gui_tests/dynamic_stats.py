...
class StatsWin:
  ...
  def __init__(self, 
    ...
    self.buffer = find_buf(session, "stats_buffer") or create_buf(session, "stats_buffer")
    
    self.window = find_window_from_title(session, "StatsWindow") or create_window(self.session, self.buffer, False, window_config or config_gen(session, config='stats', win=relative_to_win), "StatsWindow")
  ...
  def set_winner_displayable_stats(self, winner: Literal["white", "black", "draw"], msg: str):
    if winner == "white":
      self.score = "1-0"
    elif winner == 'black':
      self.score = "0-1"  
  ...
  def set_times(self, wtime:  datetime | int | None = None, btime: datetime | int | None = None):
    ...
    if wtime:
      self.wtime = self._timems_to_timestring(to_millis(wtime) if type(wtime) is not int else wtime)
    if btime:
      self.btime = self._timems_to_timestring(to_millis(btime) if type(btime) is not int else btime)
