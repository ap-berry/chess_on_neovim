...
class BoardWin():
  def __init__(self,
    ...
    self.buffer = find_buf(session, "board_buffer") or create_buf(session, "board_buffer")
    
    self.window = find_window_from_title(session, "BoardWindow") or create_window(self.session, self.buffer, True, window_config or config_gen(session, config='board', win=relative_to_win), "BoardWindow")
  ...
  def draw_push_move(self, uci_move: str):
    ...
    buf_set_lines(self.session, self.buffer, [newstartingline], startingpos['row'], startingpos['row']+1)
    ...
    buf_set_lines(self.session, self.buffer, [newendingline], endingpos['row'], endingpos['row']+1)
  ...
  def _is_weird_move(self, move: Move):
    ...
    if self.board.is_castling(move) or self.board.is_en_passant(move) or move.promotion:
      return True
    else:
      return False
